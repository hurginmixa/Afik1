...
function getBGColor()
{
        ...
        if (szRet == "") {
          szRet = idEditbox.document.body.bgColor;
        }
        return szRet
}
	...
	switch(szHow) {
		case "BackColor":
			...
			if (oSel.parentElement != null) {
				...
				if (!el)
					el = _CUtil_GetElement(oSel.parentElement(),"TH")
				if (!el)
					el = _CUtil_GetElement(oSel.parentElement(),"TR")
				if (!el)
					el = _CUtil_GetElement(oSel.parentElement(),"TABLE")
			} else {
		...
		case "Justify":
			if (oBlock) {
				...
				if ((oBlock.tagName=="HR") || ((oBlock.tagName=="IMG") && szValue!="Center")) {
					break;
				}
			}
			szHow=szHow+szValue
...
function _pageReady()
{
	idEditbox.document.body.oncontextmenu = new Function("return true")
	if (g_state.szSearch!="") {
